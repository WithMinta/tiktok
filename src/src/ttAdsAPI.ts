...
import {
    ...
    IGetVideosInfoData,
    ITikTokGetCreativeReportResponse,
...
export default class TiktokAdsAPI {
    ...
    // https://ads.tiktok.com/marketing_api/docs?id=1740662135093314
    async getCreativeReport(tiktokData: IGetCreativeReportData): Promise<ITikTokGetCreativeReportResponse> {
        const payload = {
            advertiser_id: tiktokData.advertiser_id,
            material_type: tiktokData.material_type,
            lifetime: true,
            filtering: tiktokData.filtering,
            metrics_fields: tiktokData.metrics_fields ? tiktokData.metrics_fields : ["spend", "cpc", "cpm", "impressions", "clicks", "conversion", "cost_per_conversion", "ctr", "video_play_actions", "video_watched_2s", "video_watched_6s", "average_video_play"]
        };

        const response = await axios.get(`${ TIKTOK_ADS_API }/${API_VERSION}/creative/report/get/`,
